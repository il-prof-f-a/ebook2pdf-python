...
Write-Host "Ebook2PDF - installazione asset OCR locali"
...
$PdfLibDir = Join-Path $ExtensionDir "lib\pdf-lib"
$TessdataDir = Join-Path $ExtensionDir "tessdata"

New-Item -ItemType Directory -Force -Path $TesseractDir, $CoreDir, $PdfLibDir, $TessdataDir | Out-Null
...
Copy-Item (Join-Path $NodeModules "pdf-lib\dist\pdf-lib.min.js") $PdfLibDir -Force
...
Write-Host "Incluso pdf-lib per comporre immagine originale e layer text-only Tesseract." -ForegroundColor Green
